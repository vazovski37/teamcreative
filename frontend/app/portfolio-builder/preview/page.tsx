...
export default function PreviewPage() {
    ...
    if (project.content && project.content.length > 0) {
        return (
            <div className="min-h-screen bg-black text-white selection:bg-primary/30 font-sans">
                ...
                <div className="bg-blue-600 text-center py-2 text-xs font-bold uppercase tracking-widest sticky top-0 z-[100] shadow-xl">
                    Builder Preview Mode
                </div>
                {project.content.map((block, idx) => {
                    switch (block.type) {
                        case 'hero': return <HeroBlock key={idx} {...block} />;
                        case 'text-highlight': return <TextBlock key={idx} text={block.text} label={block.label} align={block.align} />;
                        case 'legacy-columns': return <LegacyColumnsBlock key={idx} {...block} />;
                        case 'info-card': return <InfoCardBlock key={idx} {...block} />;
    ...
    // Fallback Legacy Layout Logic from normal page
    ...
    return (
        <div className="min-h-screen bg-black text-white selection:bg-blue-500/30 font-sans">
            <div className="bg-yellow-600 text-center py-2 text-xs font-bold uppercase tracking-widest sticky top-0 z-[100] shadow-xl">
                Builder Preview Mode (Legacy Layout)
            </div>
